...
func createAllIterator(ctx context.Context, primaryIter parquetquery.Iterator, conditions []traceql.Condition, allConditions bool, start, end uint64, rgs []parquet.RowGroup,
	...
	spanIter, err := createSpanIterator(makeIter, makeNilIter, innerIterators, catConditions.span, allConditions, dc, selectAll, spanSampler)
	if err != nil {
...
func (c *eventCollector) KeepGroup(res *parquetquery.IteratorResult) bool {
	...
	for _, e := range res.Entries {
		...
		default:
			// This is a null value, indicating the attribute doesn't exist
			if e.Value.IsNull() {
				ev.attrs = append(ev.attrs, attrVal{newEventAttr(e.Key), traceql.NewStaticString("nil")})
			}
		}
	}
...
func (c *linkCollector) KeepGroup(res *parquetquery.IteratorResult) bool {
	...
	for _, e := range res.Entries {
		...
		default:
			// This is a null value, indicating the attribute doesn't exist
			if e.Value.IsNull() {
				l.attrs = append(l.attrs, attrVal{newLinkAttr(e.Key), traceql.NewStaticString("nil")})
			}
		}
	}
